...
<body style="height: 100vh; display: flex; flex-direction: column;">
    ...
    <main class="d-flex flex-wrap overflow-y-auto bg-secondary">
        <div class="col-12 col-lg-8 h-75" style="background-image: url(Gemini_Generated_Image_m2odv2m2odv2m2od.png); background-repeat: repeat-y;">
        ...
        <div class="col-11 m-4 rounded-4 col-lg-3 py-5 p-2 fs-3" style="background-color: #d9d9d9; height: fit-content;">
<?php echo $produkt['specyfikacja'];?>
        </div>
        <div class="col-12 col-lg-8 bg-secondary p-5 fs-1 justify-content-evenly">
            <b class="col-8"><?php echo number_format($produkt['cena'], 2, '.', ' ');?> zł</b> <p>Stan: <?php
            if($produkt['status'] < 10){
                echo "<i style='color: red;'>" . $produkt['status']."</i>";
            }else if($produkt['status'] <=0 ){
                echo "<i style='color: red;'>Produkt niedostępny</i>";
            }else{
                echo "<i>".$produkt['status']."</i>";
            }
            ?>
            </p> <button id="Dodaj"  
            <?php
            if($produkt['status'] > 0){
                echo 'class=" rounded-5 text-center text-dark border-0 fs-2 p-2 col-12 powieksz" style="background-color: rgb(122, 226, 80);"';
            }else{
                echo 'disabled class=" rounded-5 text-center text-dark border-0 fs-2 p-2 col-12"';
            }
            ?>
